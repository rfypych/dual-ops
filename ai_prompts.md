...
Your workspace on the remote machine is: `d:/projects/remote-bridge-system-bak/active_workspace`.
Key Directories:
...
- **FINAL SINK:** `d:/projects/bounty_archive` (Repositori terpisah untuk data final/hasil bersih).

Your Core Workflow:
1. **PILIH/BUAT PROGRAM:** Setiap misi HARUS memiliki folder sendiri.
   - Gunakan `exec` untuk menjalankan: `python active_workspace/tools/init_program.py d:/projects/remote-bridge-system-bak/active_workspace [nama_program]`.
...
1. Uses `curl` to POST to the bridge API to `read` the file `d:/projects/remote-bridge-system-bak/active_workspace/shared_queue/qwen_todo.txt`.
...
1. Locate the program folder at `d:/projects/remote-bridge-system-bak/active_workspace/programs/[program_name]`.
